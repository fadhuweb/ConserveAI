"""Train a supervised LSTM for multi-threat forecasting.

Architecture: 2-layer LSTM → dropout → 3 independent sigmoid heads.
...
Outputs:
  results/models/lstm_supervised/model.pkl
  results/models/lstm_supervised/hyperparameters.json
  results/models/lstm_supervised/test_metrics.json
"""
...
LSTM_MODEL_DIR = MODELS_DIR / "lstm_supervised"
...
class ThreatLSTM(nn.Module):
    def __init__(self, n_features: int, hidden_size: int = 128,
                 n_layers: int = 2, dropout: float = 0.3, n_threats: int = 3):
        super().__init__()
        self.lstm = nn.LSTM(
        ...
        self.heads = nn.ModuleList([nn.Linear(hidden_size, 1) for _ in range(n_threats)])

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        out, _ = self.lstm(x)            # (B, seq_len, hidden)
        last   = self.drop(out[:, -1, :])  # (B, hidden)
        return torch.cat([h(last) for h in self.heads], dim=1)  # (B, 3) logits
...
def train_lstm(X_train, Y_train, X_val, Y_val, device: torch.device):
    ...
    print(f"  Architecture: LSTM hidden={HPARAMS['hidden_size']} "
          f"layers={HPARAMS['n_layers']} dropout={HPARAMS['dropout']}")
...
@torch.no_grad()
def predict_proba(model: ThreatLSTM, X: np.ndarray, device: torch.device,
...
def run():
    ...
    print("\n=== LSTM ===")
    model = train_lstm(X_train, Y_train, X_val, Y_val, device)
    ...
    with open(RESULTS_DIR / "lstm_supervised_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    with open(LSTM_MODEL_DIR / "test_metrics.json", "w") as f:
        json.dump(test_metrics, f, indent=2, default=str)
    with open(LSTM_MODEL_DIR / "hyperparameters.json", "w") as f:
        json.dump(HPARAMS, f, indent=2)
    with open(LSTM_MODEL_DIR / "model.pkl", "wb") as f:
        pickle.dump({
    ...
    print("Saved to results/models/lstm_supervised/")
