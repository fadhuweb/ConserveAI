"""FixMatch Transformer for multi-threat semi-supervised forecasting.

Same FixMatch algorithm as train_lstm_fixmatch.py but using the ThreatTransformer
architecture (sinusoidal positional encoding, mean pooling, 2 encoder layers).

See train_lstm_fixmatch.py for full algorithm description and augmentation rationale.

Outputs:
  results/models/transformer_fixmatch/model.pkl
  results/models/transformer_fixmatch/test_metrics.json
  results/transformer_fixmatch_results.json
"""
...
from supervised.train_transformer import ThreatTransformer, EarlyStopping, masked_weighted_bce, predict_proba

TRANSFORMER_FM_DIR = MODELS_DIR / "transformer_fixmatch"
CONF_THRESHOLD     = 0.95
LAMBDA_U           = 1.0
WEAK_NOISE         = 0.05
STRONG_NOISE       = 0.15
...
def consistency_loss(logits_s: torch.Tensor, pseudo: torch.Tensor) -> torch.Tensor:
    ...
    for i in range(pseudo.shape[1]):
        total += nn.BCEWithLogitsLoss()(logits_s[:, i], pseudo[:, i])
    return total
    ...
    for epoch in range(1, HPARAMS["max_epochs"] + 1):
        ...
        for xb, yb in train_loader:
            ...
            if use_ul:
                ...
                with torch.no_grad():
                    probs_w = torch.sigmoid(model(X_weak))

                conf  = torch.max(probs_w, 1 - probs_w)
                gate  = conf.min(dim=1).values > CONF_THRESHOLD
...
def run():
    ensure_dirs()
    TRANSFORMER_FM_DIR.mkdir(parents=True, exist_ok=True)
    ...
    with open(RESULTS_DIR / "transformer_fixmatch_results.json", "w") as f:
        json.dump(results, f, indent=2, default=str)
    with open(TRANSFORMER_FM_DIR / "test_metrics.json", "w") as f:
        json.dump(test_metrics, f, indent=2, default=str)
    with open(TRANSFORMER_FM_DIR / "model.pkl", "wb") as f:
        pickle.dump({
    ...
    print("\nTransformer FixMatch Test Results:")
    ...
    print("Saved to results/models/transformer_fixmatch/")
